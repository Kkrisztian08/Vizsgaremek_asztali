...
public class ProgramApplicationApi {
    ...
    private static final String API_URL = BASE_URL + "/api/application";
    private static Gson jsonConverted = new Gson();

    public static List<ProgramApplication> get() throws IOException {
        String json = Api.get(API_URL);
        Type type = new TypeToken<List<ProgramApplication>>(){}.getType();
        return jsonConverted.fromJson(json,type);
    }
    public static ProgramApplication post(ProgramApplication uj) throws IOException {
        String ujJson = jsonConverted.toJson(uj);
        String json = Api.post(API_URL, ujJson);
        return jsonConverted.fromJson(json, ProgramApplication.class);
    }
    ...
    public static ProgramApplication put(ProgramApplication modosit) throws IOException {
